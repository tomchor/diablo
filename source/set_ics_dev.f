...
!       END DO
       DO J=0,NY
         DO K=0,NZP-1
           DO I=0,NXM
             U1(I,K,J)=0.d0
             U2(I,K,J)=0.d0
             if (GYF(j).lt.-15.d0) then
             U3(I,K,J)=(GYF(J)+15.d0)*DRHODX(1)*(
     &             (LX/(2.d0*Lf))*
     &           (1.d0-(TANH((GX(I)-LX/2.d0)/Lf))**2.d0)
     &            /(0.5d0*(TANH((LX-LX/2.d0)/Lf)
     &                    -TANH((0.d0-LX/2.d0)/Lf)))
     &              -1.d0)*RI(1)/I_RO
             else
             U3(I,K,J)=0.5d0*(GYF(J)+15.d0)*DRHODX(1)*(
     &             (LX/(2.d0*Lf))*
     &           (1.d0-(TANH((GX(I)-LX/2.d0)/Lf))**2.d0)
     &            /(0.5d0*(TANH((LX-LX/2.d0)/Lf)
     &                    -TANH((0.d0-LX/2.d0)/Lf)))
     &              -1.d0)*RI(1)/I_RO
         ...
         END DO
       END DO
! For unbalanced initial conditions
!       DO J=0,NY
!         DO K=0,NZP-1
!           DO I=0,NXM
!             U1(I,K,J)=0.d0
!             U2(I,K,J)=0.d0
!             U3(I,K,J)=-0.2d0*DRHODX(1)*RI(1)/I_RO*(GYF(J)+LY/2.d0)
!           END DO
!         END DO
!       END DO
      else
...
! For Front case, specify given RI_B profile
       DO K=0,NZP-1
         DO I=0,NXM
          TH(I,K,0,N)=0.d0
           DO J=1,NY
! For EXPORT simulations
!             if (GYF(J).lt.-300.d0) then
!               RI_B(J)=100.d0
!               TH(I,K,J,N)=(GYF(J)+800.d0)*
!     &                    RI_B(J)*(RI(N)*DRHODX(N))**2.d0
!     &                    /I_RO**2.d0/RI(N)
!             else
!                   RI_B(J)=1.0d0
!                   TH(I,K,J,N)=(GYF(J)+300.d0)*
!     &                    RI_B(J)*(RI(N)*DRHODX(N))**2.d0
!     &                    /I_RO**2.d0/RI(N)
!     &                 +(-300.d0+800.d0)*100.d0*(RI(N)*DRHODX(N))**2.d0
!     &                    /I_RO**2.d0/RI(N)
!             end if
! For front_io simulations
             if (GYF(J).lt.-25.d0) then
               RI_B(J)=3.d0
               TH(I,K,J,N)=(GYF(J)+40.d0)*
     &                    RI_B(J)*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
             else if (GYF(J).lt.-15.d0) then
                   RI_B(J)=6.0d0
                   TH(I,K,J,N)=(GYF(J)+25.d0)*
     &                    RI_B(J)*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
     &                 +(-25.d0+40.d0)*3.d0*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
             else
                   RI_B(J)=0.0d0
                   TH(I,K,J,N)=(GYF(J)+15.d0)*
     &                    RI_B(J)*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
     &                 +(-25.d0+40.d0)*3.d0*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
     &                 +(-15.d0+25.d0)*6.d0*(RI(N)*DRHODX(N))**2.d0
     &                    /I_RO**2.d0/RI(N)
             end if
! Add isolated front
             Lf=2.d3
             TH(I,K,J,N)=TH(I,K,J,N)+
     &        (LX*DRHODX(N)/2.d0)*TANH((GX(I)-LX/2.d0)/Lf)
     &        /(0.5d0*(TANH((LX-LX/2.d0)/Lf)-TANH((0.d0-LX/2.d0)/Lf)))
     &        -DRHODX(N)*GX(I)
...
! Passive tracers
       DO K=0,NZP-1
         DO I=0,NXM
           DO J=1,NY
             TH(I,K,J,N)=0.5d0*(1.d0+tanh((GYF(J)+50.d0)/20.d0))
           END DO
         END DO
       END DO
      END IF
